...
export const ScrollSection = ({
  ...
  return (
    <section
      ref={ref}
      id={id}
      // 1. REMOVED overflow-hidden so the shadow can actually cast downwards
      className={`relative min-h-screen w-full bg-transparent flex flex-col justify-between border-t pt-1 ${bgColor}`}
    >
      {/* THE FLASHING TOP BORDER */}
      <div
        //  2. ENHANCED SHADOW: Changed to a heavy, bright colored glow (adjust rgba color to match your design)
        className={`absolute top-0 left-0 right-0 h-1 transition-all duration-700 ease-in-out ${
          isInView
            ? "animate-border-flash opacity-100"
      ...
      {/* 3. ADDED overflow-hidden here instead, so your section content stays contained without clipping the top border shadow */}
      <div className="w-full h-full flex flex-col justify-center items-center my-auto overflow-hidden">
        {children}
      </div>
    </section>
  );
